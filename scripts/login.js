...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    // Populate the dropdown with real users OLD 
    ...
    // PERSISTING ANY PREVIOUS EDIT CHANGES TO PROFILE 
    let usersFromStorage = JSON.parse(localStorage.getItem("users"));

    if (!usersFromStorage) {
      // First time - load from JSON and save to localStorage
      const response = await fetch("data/users.json");
      allUsers = await response.json();
      localStorage.setItem("users", JSON.stringify(allUsers));
      usersFromStorage = allUsers;
    } else {
      // Use localStorage data (has profile updates)
      allUsers = usersFromStorage;
    }

    // Populate the dropdown with users from localStorage (updated data)
    allUsers.forEach((user) => {
      if (user.isActive) {
        const option = document.createElement("option");
        option.value = user.userId;
  ...
  loginForm.addEventListener("submit", (e) => {
    ...
    //Pulling the data from local storage to persist any previous edit changes
    const usersFromStorage =
      JSON.parse(localStorage.getItem("users")) || allUsers;
    const userFromStorage = usersFromStorage.find(
      (u) => u.userId === parseInt(selectedUserId)
    );
    ...
    const userData = {
      ...
      department: userFromStorage.department,
    ...
    if (userData.department && userData.department.toLowerCase() == "admin") {
